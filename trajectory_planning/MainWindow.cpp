...
MainWindow::~MainWindow() {
	...
	if (planworker && planworker->isBusy()) planworker->requestAbort();
	if (m_thread_planwork) {
		m_thread_planwork->quit();
		m_thread_planwork->wait(3000);
		delete m_thread_planwork;
		m_thread_planwork = nullptr;
	}
	planworker = nullptr;
...
void MainWindow::connect_init() {
	...
	connect(ui->actionStart, &QAction::triggered, this, &MainWindow::actionStart_triggered);
...
void MainWindow::initSimulation() {
	...
	// ---- 起点 -> 预焊接点 轨迹规划 worker(替代原 Thread) ----
	planworker->moveToThread(m_thread_planwork);
	QObject::connect(m_thread_planwork, &QThread::finished, planworker, &QObject::deleteLater);

	// robotWidgetInit() 已在构造函数中先于 initSimulation() 执行, 规划上下文此时已就绪
	planworker->setContext(this->mdl, this->model, this->planner, this->optimizer, &this->mutex);

	connect(planworker, &PlanWorker::started, this, [=]() {
		// 规划期间 worker 独占 mdl/model/scene, 暂停场景刷新避免与 occtUpdate 竞争
		flushSceneTimer->stop();
		ui->trajProgressBar->setRange(0, 100);
		ui->trajProgressBar->setValue(0);
		}, Qt::QueuedConnection);

	connect(planworker, &PlanWorker::progress, this, [=](int percent) {
		ui->trajProgressBar->setValue(percent);
		}, Qt::QueuedConnection);

	connect(planworker, &PlanWorker::finished, this, [=](const std::vector<rl::math::Vector>& jointTrajectory,
		double plannerMs) {
			flushSceneTimer->start(50);
			if (jointTrajectory.size() < 1) { return; }
			...
			ui->console->print(ct::LOG_INFO, QString("Trajectory start to pre-weld point done in %1 ms, %2 points.")
				.arg(QString::number(plannerMs, 'f', 1)).arg(jointTrajectory.size()));

			// 链式触发焊缝段逆解, 衔接点为规划轨迹的末端关节角
	...
	connect(planworker, &PlanWorker::failed, this, [=](const QString& reason) {
		flushSceneTimer->start(50);
		ui->console->print(ct::LOG_ERROR, reason);
		}, Qt::QueuedConnection);

	connect(planworker, &PlanWorker::message, this, [=](int level, const QString& msg) {
		switch (level)
		{
		case 0:
			ui->console->print(ct::LOG_INFO, msg);
...
void MainWindow::execSimulation(int& inter) {
	...
	this->model->setPosition(current_joint);
...
void MainWindow::Trajectory() {
	...
	if (mergedTraj.size() > 2) {
		if (planworker->isBusy()) {
			ui->console->print(ct::LOG_WARNING, tr("Trajectory planning is already running!"));
			return;
		}

		// 起点 -> 预焊接点: 基于 loadRobotWidget 初始化的 AddRrtConCon + AdvancedOptimizer
		// 做关节空间无碰撞规划, 结果以 std::vector<rl::math::Vector> 回传主线程
		PlanToPreWeldParams p;
		p.q_start = this->mdl->getHomePosition();       // 亦可用 this->mdl->getPosition() 从当前位姿出发
		p.preWeldPoint = mergedTraj[0];
		p.T_flange_to_tcp = tcp_transform;
		p.preWeldBackOffset = 50.0;                     // 沿焊接系 -Z 后退 50mm 作为预焊接点
		p.interpStep = 1.0;                             // 播放密度, 与 model->distance 同量纲
		p.plannerTimeoutSec = 30.0;
		p.optimize = true;
		p.ikTimeoutMs = 500;

		QMetaObject::invokeMethod(planworker, "doPlanToPreWeld", Qt::QueuedConnection, Q_ARG(PlanToPreWeldParams, p));
	}
}
