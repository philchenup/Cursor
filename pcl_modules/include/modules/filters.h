...
namespace ct
{
    ...
    {
    public:
        Filters() : cloud_(nullptr), negative_(false) {}
         ...
         */
        void VoxelGrid(float lx, float ly, float lz, Cloud::Ptr& cloud_out, float& time);

        /**
         * @brief 在给定的 PointCloud 上组装本地 3D 网格，并对数据进行下采样 + 过滤
         * @param lx ly lz 设置体素网格叶大小
         */
        void ApproximateVoxelGrid(float lx, float ly, float lz, Cloud::Ptr& cloud_out, float& time);
         ...
         */
        void StatisticalOutlierRemoval(int nr_k, double stddev_mult, Cloud::Ptr& cloud_out, float& time);
         ...
         */
        void RadiusOutlierRemoval(double radius, int min_pts, Cloud::Ptr& cloud_out, float& time);

        /**
         * @brief 条件滤波器
         * @param con 设置过滤器将使用的条件
         */
        void ConditionalRemoval(ConditionBase::Ptr con, Cloud::Ptr& cloud_out, float& time);

        /**
         * @brief 在给定的 PointCloud 上组装一个本地 2D 网格，并对数据进行下采样
         * @param resolution 设置网格分辨率
         */
        void GridMinimum(float resolution, Cloud::Ptr& cloud_out, float& time);

        /**
         * @brief 通过消除局部最大值来对云进行下采样
         * @param radius 设置用于确定点是否为局部最大值的半径
         */
        void LocalMaximum(float radius, Cloud::Ptr& cloud_out, float& time);

        /**
         * @brief 去除出现在边缘不连续处的鬼点
         * @param threshold 设置阴影点拒绝的阈值
         */
        void ShadowPoints(float threshold, Cloud::Ptr& cloud_out, float& time);

        /**
         * @brief 对数据进行下采样过滤
         * @param radius 设置 3D 网格叶子大小
         */
        void DownSampling(float radius, Cloud::Ptr& cloud_out, float& time);

        /**
         * @brief 在给定的 PointCloud 上组装一个本地 3D 网格，并对数据进行下采样+ 过滤
         * @param radius 设置 3D 网格叶子大小
         */
        void UniformSampling(float radius, Cloud::Ptr& cloud_out, float& time);
         ...
         */
        void RandomSampling(int sample, int seed, Cloud::Ptr& cloud_out, float& time);
         ...
         */
        void ReSampling(float radius, int polynomial_order, Cloud::Ptr& cloud_out, float& time);
         ...
         */
        void SamplingSurfaceNormal(int sample, int seed, float ratio, Cloud::Ptr& cloud_out, float& time);
         ...
         */
        void NormalSpaceSampling(int sample, int seed, int bin, Cloud::Ptr& cloud_out, float& time);
         ...
         */
        void MedianFilter(int window_size, float max_allowed_movement, Cloud::Ptr& cloud_out, float& time);

        /**
         * @brief 3D 平面剪裁器
         * @param plane_params 设置新的平面参数
         */
        void PlaneClipper3D(const Eigen::Vector4f& plane_params, Cloud::Ptr& cloud_out, float& time);
