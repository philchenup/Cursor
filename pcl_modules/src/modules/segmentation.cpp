...
namespace ct
{
                                       ...
                                       std::vector<Cloud::Ptr>& clouds, float& time)
    {
        ...
        clouds = getClusters(cluster_indices);
        time = timer.toc();
    }

    void Segmentation::ExtractPolygonalPrismData(const Cloud::Ptr& hull, double height_min, double height_max,
                                          ...
                                          std::vector<Cloud::Ptr>& clouds, float& time)
    {
        ...
        const Eigen::Vector3f c = cloud_->center();
        PointXYZRGBN seed;
        seed.x = c[0];
        seed.y = c[1];
        seed.z = c[2];
        foreground_points->push_back(seed);
                                             ...
                                             std::vector<Cloud::Ptr>& clouds, float& time)
    {
        ...
        clouds = getClusters(indices_out);
        time = timer.toc();
    }
