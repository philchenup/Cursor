...
namespace ct
{
    void Surface::EarClipping(PolygonMesh::Ptr& surface, PolygonMesh::Ptr& mesh, float& time)
    {
        TicToc timer;
        timer.tic();
    ...
    void Surface::MarchingCubesHoppe(float iso_level, int res_x, int res_y, int res_z, float percentage,
                                     float dist_ignore, PolygonMesh::Ptr& mesh, float& time)
    {
        TicToc timer;
        timer.tic();
        pcl::search::KdTree<PointXYZRGBN>::Ptr tree(new pcl::search::KdTree<PointXYZRGBN>);
    ...
    void Surface::ConvexHull(bool value, int dimensio, PolygonMesh::Ptr& mesh, float& time)
    {
        TicToc timer;
        timer.tic();
        pcl::search::KdTree<PointXYZRGBN>::Ptr tree(new pcl::search::KdTree<PointXYZRGBN>);
        ...
        sur.reconstruct(*result);
        mesh = result;
        time = timer.toc();
    }

    void Surface::ConcaveHull(double alpha, bool value, int dimensio, PolygonMesh::Ptr& mesh, float& time)
    {
        TicToc timer;
        timer.tic();
        pcl::search::KdTree<PointXYZRGBN>::Ptr tree(new pcl::search::KdTree<PointXYZRGBN>);
