...
配准得到多个抓取目标后，可用 PCL 模块 [`filterreg_rigid/stack_filter`](filterreg_rigid/stack_filter) 做堆叠过滤。视角从 −Z 朝 +Z 看（Z 越小越靠上），只丢掉被更近目标压住的物体；旁边空着的目标即使更远也保留。提供 2D 投影与 3D 包围盒两种方法。
