...
import numpy as np
import trimesh
from utils.win_compat import configure_opengl_platform

# EGL is Linux-only. Upstream always set PYOPENGL_PLATFORM=egl, which crashes
# pyrender on Windows. Configure the backend before importing pyrender.
...
class PyrenderRasterizer(renderer_base.RendererBase):
    ...
    def get_object_model(
        ...
        **kwargs: Any,
        ...
        if "itodd" in object_model_path:
            mesh_color = structs.Color(0.5, 0.5, 0.5)  # Default color for iTODD models.
    ...
    def add_object_model(
        ...
        **kwargs: Any,
        ...
        if background is None:
            ...
            bg_color = np.array(background, dtype=np.float32)
            bg_color = np.concatenate((bg_color, [0.0]))
        if model_path is not None:
            if "itodd" in model_path or "tless" in model_path and mesh_color is None:
        ...
        if obj_id not in self.object_meshes:
            ...
            if mesh_color:
                ...
                logger.info("Colored the model with: {}".format(mesh_color))
            self.object_meshes[obj_id] = trimesh_model
        else:
        ...
        self.object_scenes[obj_id] = pyrender.Scene(
            bg_color=bg_color, ambient_light=ambient_light
        )
    ...
    def render_object_model(
        ...
        **kwargs: Any,
        ...
        if obj_id not in self.object_scenes:
            self.add_object_model(obj_id, background=background)
        ...
        if return_tensors:
            for name in output.keys():
                if output[name] is not None:
                    output[name] = torch_helpers.array_to_tensor(output[name])
