 ...
 *
 * Pipeline:
 *   1. Read STEP ¡ú B-Rep shape
 *   2. Collect cylindrical faces matching target diameter ¡À tolerance
 ...
 *   5. Snap each hole axis to the nearest workpiece ¡ÀX / ¡ÀY / ¡ÀZ
 *   6. Mesh B-Rep in memory ¡ú PCL PolygonMesh ¡ú area-weighted sampling
 */
...
public:
    ...
    /** Linear deflection for BRep meshing (mm). Smaller ¡ú finer mesh. */
    ...
    const ct::Cloud::Ptr& pointCloud() const { return cloud_; }

    const pcl::PolygonMesh& polyMesh() const { return mesh_; }
...
private:
    ...
    /**
     * Mesh B-Rep in memory ¡ú pcl::PolygonMesh ¡ú CAD2PCD area-weighted sampling.
