...
namespace {
    ...
    float triangleArea(const pcl::PointXYZRGBNormal& a,
        const pcl::PointXYZRGBNormal& b,
        const pcl::PointXYZRGBNormal& c) {
     ...
     */
    ct::Cloud::Ptr sampleMeshSurface(
        const pcl::PolygonMesh& mesh,
        std::size_t num_samples) {
        ct::Cloud::Ptr vertices(new ct::Cloud);
...
TopoDS_Shape CylindricalHoleDetector::extractSolid(
    ...
    explorer.Next();
    // Unique solid ¡ú use it for classification; otherwise keep root shape.
    if (!explorer.More()) {
        return first;
    }
    return shape;
...
bool CylindricalHoleDetector::isHoleCylinder(
    ...
    if (state == TopAbs_OUT) {
        return true;  // void on axis ¡ú cavity / hole
    }
    if (state == TopAbs_IN) {
        return false;  // material on axis ¡ú boss
    }
...
std::vector<HoleInfo> CylindricalHoleDetector::mergeCoaxial(
    ...
    for (const auto& cluster : clusters) {
        ...
        // Area-weighted average of axis locations ¡ú more stable hole coordinate.
...
gp_Dir CylindricalHoleDetector::snapToWorkpieceAxis(const gp_Dir& direction) {
    // Align hole axis with the nearest workpiece principal axis (¡ÀX / ¡ÀY / ¡ÀZ).
...
bool CylindricalHoleDetector::samplePointCloud() {
    ...
    // 2) OCCT triangulation ¡ú PCL PolygonMesh (no STL file I/O)
    if (!shapeToPolygonMesh(shape_, mesh_)) {
        last_error_ = "Failed to build in-memory triangle mesh from B-Rep";
        return false;
    }

    // 3) CAD2PCD area-weighted surface sampling
    ct::Cloud::Ptr sampled = sampleMeshSurface(mesh_, sample_count_);
...
bool CylindricalHoleDetector::savePointCloud(const std::string& pcd_path) const {
    ...
    return pcl::io::savePLYFileBinary(pcd_path, *cloud_) == 0;
}