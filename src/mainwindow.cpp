...
void MainWindow::connect_init() {
	...
	/*connect(ui->actionOpen, &QAction::triggered, this, &MainWindow::load3DModel);
	connect(ui->actionRemove, &QAction::triggered, this, &MainWindow::removeModel);
...
void MainWindow::load3DModel() {
	...
	ReadModel rm;
	if (suffix == "stl") {
		this->myOccView->loadSceneShape = ReadModel::readStlModel(filename.toStdString().c_str());
	}
	else if (suffix == "step") {
		this->myOccView->loadSceneShape = ReadModel::readStepModel(filename.toStdString().c_str());
	}
	else {
	...
	AIS_Shape* ais_shape = new AIS_Shape(this->myOccView->loadSceneShape);
	loadWrlModel(wrlFilename.toStdString());

	myOccView->getContext()->Display(ais_shape, Standard_False);
	if (isTheFirstDraw) {
		myOccView->fitAll();
		isTheFirstDraw = false;
	}
...
void MainWindow::Trajectory() {
	...
	params.q_initial = this->mdl->getPosition();
	if (params.q_initial.size() == 0) {
		params.q_initial = this->mdl->getHomePosition();
	}
	params.constrainRail = true;
	params.railWindow = 10;
	params.timeoutMs = 500;
	...
	// trajProgressBar
	/*Handle(AIS_Shape) aisWire = new AIS_Shape(wires[0]);
	aisWire->SetColor(Quantity_NOC_BLUE);
	aisWire->SetDisplayMode(AIS_WireFrame);
	aisWire->SetWidth(3.0);
	this->myOccView->getContext()->Display(aisWire, Standard_True);*/
}
