...
namespace poser {
	...
	*/
	...
	__host__ __forceinline__ float norm_inversed(const float4& vec) {
		return 1.0f / sqrtf(squared_norm(vec));
	}
    ...
    __device__ __forceinline__ void normalize(float3& vec) {
        const float inv_vecnorm = rsqrtf(squared_norm(vec));
        vec.x *= inv_vecnorm;
        vec.y *= inv_vecnorm;
        vec.z *= inv_vecnorm;
	...
	__host__ __forceinline__ void normalize(float3 &vec) {
		const float inv_vecnorm = 1.0f / sqrtf(squared_norm(vec));
		vec.x *= inv_vecnorm;
		vec.y *= inv_vecnorm;
		vec.z *= inv_vecnorm;
	...
	*/
	...
	__host__ __device__ __forceinline__ bool vertex_in_aabb(
		const float4& vertex,
		const float3& aabb_min,
		const float3& aabb_max
	) {
		if(vertex.x > aabb_max.x || vertex.y > aabb_max.y || vertex.z > aabb_max.z)
			return false;
		if(vertex.x < aabb_min.x || vertex.y < aabb_min.y || vertex.z < aabb_min.z)
			return false;
		return true;
	}
}